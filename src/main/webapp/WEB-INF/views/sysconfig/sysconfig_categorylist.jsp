...
<div id="tabs" class="ui-tabs ui-widget ui-widget-content ui-corner-all">
	...
	<form:form modelAttribute="searchForm" action="${REQUEST_URI}" class="form-inline" >
    	类型 
    	<form:select path="form['itemTypeKey_eq']" id="search_itemTypeKey">
    		<form:options items="${item_type}" itemLabel="dictValue0" itemValue="dictKey"/>
    	</form:select>
    	<form:input path="form['cateName_blk']" id="search_dictType" placeholder="名称" cssClass="input_text" />
	...
	<table class="tbl_l"> 
	    	...
	    	<tr>
		    	...
		    	<td>
		    		<a href="${pageContext.request.contextPath}/sysConfig/category/${entry.cateId}/update" class="btn btn-small">修改</a> 
		    		<a href="${pageContext.request.contextPath}/sysConfig/category/${entry.cateId}/remove" class="btn btn-small" onclick="return confirm('确定要删除吗？')">删除</a> 
		    	</td>
	    	</tr>	
	    	</c:forEach>
	    </table> 
		...
		<div class="paging"> 
		    ...
		    <div class="sys_btnBox">
				<input type="button" onclick="window.location='${pageContext.request.contextPath}/sysConfig/category/create'" value="创建新系列">
			</div>
		</div>	
	</div>
</div>